      ...
      <img src="assets/images/backgrounds/signup-bg.png" class="w-full h-full object-cover" alt="background">
                ...
                @if($errors->any())
                    @foreach($errors->all() as $error)
                        <div class="bg-red-500" style="color: red; background: blue;">
                            {{$error}}
                        </div>
                    @endforeach
                @endif

    <form method="POST" action="{{ route('register') }}" class="flex flex-col justify-between flex-1 px-[18px] pt-8 pb-6" enctype="multipart/form-data">
        @csrf
